...
class FTB_PT_Rotator_Panel(Panel):
    ...
    bpy.types.WindowManager.fMinRotation = bpy.props.FloatProperty(
        description="Minimum amount to rotate objects by", min=-359, max=359)
    bpy.types.WindowManager.fMaxRotation = bpy.props.FloatProperty(
        description="Maximum amount to rotate objects by", min=-359, max=359)
    ...
    bpy.types.WindowManager.bRandomizeDirection = bpy.props.BoolProperty(
        description="Randomize direction in which objects are rotated. (If disabled, always rotate objects according to sign +/-)")
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.prop(context.window_manager, "bRandomizeDirection",
...
def register():
    bpy.utils.register_class(FTB_PT_Rotator_Panel)


def unregister():
    bpy.utils.unregister_class(FTB_PT_Rotator_Panel)
